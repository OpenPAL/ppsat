...
sat_solver::sat_solver(cnf &m_cnf) : cnf_manager(m_cnf) {
    ...
    for (int i = 1; i <= (int) m_vc; i++) {
        if (m_vars[i].m_value == _FREE) {
            ...
            else if (m_vars[i].m_activity[_NEGA] == 0 && m_vars[i].m_activity[_POSI] > 0) {
                assert_literal(i, NULL);
            }
        }
    }
    ...
    for (unsigned i = 1; i <= m_vc; i++) {
        if (m_vars[i].m_value == _FREE && SCORE(i) > 0) {
            ...
            m_vars[i].m_phase = (m_vars[i].m_activity[_POSI] > m_vars[i].m_activity[_NEGA]) ? _POSI : _NEGA;
        }
    }
    ...
    for (unsigned i = 0; i < m_n_vars; i++) m_var_position[m_var_order[i]] = i;
    m_next_var = 0;
...
int sat_solver::select_literal() {
    ...
    for (int i = m_next_clause; i >= last_clause; i--) {
        ...
        if (d > _DT) return x;
        else if (-d > _DT) return -x;
        else return (m_vars[x].m_phase == _POSI) ? (x) : -(int) (x);
    }
        ...
        if (!decide(lit))
            do {
                ...
                if (m_n_conflicts == m_next_restart) {
                    ...
                    backtrack(1);
                    if (m_d_level != m_a_level) break;

                    // partial restart at m_a_level
                } else backtrack(m_a_level);
...
void sat_solver::print_solution(FILE *ofp) {
    for (unsigned i = 1; i <= m_vc; i++) {
        if (m_vars[i].m_value == _POSI)      fprintf(ofp, "%d ", i);
        else if (m_vars[i].m_value == _NEGA) fprintf(ofp, "-%d ", i);
    }
    fprintf(ofp, "0\n");
}

void sat_solver::print_stats() {
    printf("c %d decisions, %d conflicts, %d restarts\n", m_n_decisions, m_n_conflicts, m_n_restarts);
}
