...
cnf_manager::cnf_manager(cnf &m_cnf) {
    ...
    // create lit_pool
    int *p = m_lit_pool = (int *) calloc(m_lit_pool_capacity = (m_cnf.m_lc + m_cnf.m_cc) * 2, sizeof(int));
    if (m_lit_pool == NULL) {
    ...
    for (i = 0; i < m_cnf.m_cc; i++) {
        if (m_cnf.m_clauses[i][1] == 0) {        // unit clause
            ...
            if (FREE(lit)) set_literal(*(m_stack_top++) = lit, zero);
    ...
    for (i = 1; i <= m_vc; i++)
        for (j = 0; j <= 1; j++) {
            ...
            m_vars[i].m_imp[j] = (int *) calloc(imp[j][i].size() + 4, sizeof(int));
...
cnf_manager::~cnf_manager() {
    for (vector<int *>::iterator it = m_lit_pools.begin(); it != m_lit_pools.end(); free(*(it++)));
    while (*(--m_stack_top));
    free(m_stack_top);
    for (unsigned i = 1; i <= m_vc; i++) for (unsigned j = 0; j <= 1; j++) free(m_vars[i].m_imp[j]);
    free(m_var_order);
    free(m_var_position);
    delete[] m_vars;
}
...
bool cnf_manager::assert_literal(int lit, int *ante) {
    ...
    while (m_stack_top < newm_stack_top) {
        ...
        for (vector<int *>::iterator it = watch_list.begin(); it != watch_list.end(); it++) {
...
void cnf_manager::learn_clause(int *first) {
    ...
    for (m_tmp_conflict_lits.clear(); *first; first++) {
        ...
        if (m_vars[VAR(*first)].m_d_level == 1) continue;

        if (m_vars[VAR(*first)].m_d_level < m_d_level) m_tmp_conflict_lits.push_back(*first);
        else cur_level_lits++;
    ...
    while (true) {
        ...
        if (m_vars[var].m_ante) update_scores(m_vars[var].m_ante - 1);

        // update m_next_var
        if (m_var_position[var] < m_next_var) m_next_var = m_var_position[var];

        // UIP reached
        if (cur_level_lits-- == 1) break;

        // else, replace with antecedent (resolution)
        for (int *ante = m_vars[var].m_ante; *ante; ante++) {
            if (m_vars[VAR(*ante)].m_mark || m_vars[VAR(*ante)].m_d_level == 1) continue;
            if (m_vars[VAR(*ante)].m_d_level < m_d_level) m_tmp_conflict_lits.push_back(*ante);
            else cur_level_lits++;
    ...
    for (it = m_tmp_conflict_lits.begin(); it != m_tmp_conflict_lits.end(); it++) {
        bool redundant = true;
...
inline void cnf_manager::add_clause() {
    ...
    if (size > 1) {
        ...
        for (unsigned i = 1; i < size - 1;) m_lit_pool[m_lit_pool_size++] = m_conflict_lits[i++];
    }
